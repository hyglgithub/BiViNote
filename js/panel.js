...
(function () {
  ...
  const DEFAULT_DEEPSEEK_PROMPT = `你是一个视频内容整理助手，请将提供的「B站视频字幕」整理为简洁、高质量、适合长期保存的 Markdown 学习笔记。
...
{markdown}`;
  ...
  const DEFAULT_DEEPSEEK_SUMMARY = `你是一个视频总结助手，请根据提供的「B站视频字幕」生成简洁、准确的视频总结。
...
直接输出总结，不要输出任何额外说明。`;
  ...
  const DEFAULT_DEEPSEEK_BILI = `你是一位笔记知识点分析整理专家，请将提供的「B站视频字幕」整理成一份结构化学习笔记。
...
{markdown}`;
  ...
  function bindDocAutoEvents() {
    ...
    let organizeBvid = {};
    // promptType -> 该结果本次会话已成功保存到 B站笔记 → 按钮显示「查看笔记」（点击打开我的笔记列表）
    let noteSaved = {};
    ...
    function renderPromptSwitcher() {
      ...
      const summaryName = settings.deepseekSummaryName || '视频总结';
      const clearName = settings.deepseekPromptName || '内容整理';
    ...
    // 「记笔记」按钮文案：结果已保存到 B站笔记 → 「查看笔记」，否则「记笔记」
    function syncNoteLabel() {
      if (!noteBtn) return;
      noteBtn.textContent = noteSaved[currentPromptType] ? '查看笔记' : '记笔记';
    }
    ...
    if (noteBtn) {
      noteBtn.addEventListener('click', async () => {
        const nowBvid = window.BiViNote.state.bvid;
        // 结果已保存 → 按钮已是「查看笔记」，点击直接打开我的笔记列表
        if (noteSaved[currentPromptType]) {
          chrome.runtime.sendMessage({ type: 'bn-open-note', url: 'https://space.bilibili.com/v/note-list' });
          return;
        }
        ...
        try {
          ...
          if (out.ok) {
            // 保存成功 → 按钮随之显示为「查看笔记」
            noteSaved[currentPromptType] = true;
            syncNoteLabel();
    ...
    if (clearBtn) {
      clearBtn.addEventListener('click', async () => {
        ...
        noteSaved[currentPromptType] = false;
  ...
  function getPromptName(taskId) {
    const settings = window.BiViNote.state.settings;
    if (taskId === 'clear') return settings.deepseekPromptName || '内容整理';
    if (taskId === 'summary') return settings.deepseekSummaryName || '视频总结';
